...
> 헤어짐 상태(`DISCONNECTED`)·삭제는 이번 스프린트 범위 외 — README '안 하는 것' 참조.
...
### Couple (`couple`)
- `id`, `requester_id`(먼저 코드 입력), `target_id`(코드 주인), `status`, `connected_at`, `created_at`
- 제약: `requester_id <> target_id`, `status IN (PENDING, CONNECTED)`
- 유저당 커플 1개(상태 무관, 만료 PENDING 제외)는 **서비스 트랜잭션에서 보장**한다 — 만료 PENDING의 lazy 판정·H2 부분 인덱스 한계로 DB UNIQUE는 미적용(FR-2.5/FR-2.6).
> 헤어짐/삭제(`DISCONNECTED`, `disconnected_*`, `delete_scheduled_at`, 재결합)는 이번 스프린트 범위 외 — README '안 하는 것' 참조.

### CoupleProfile (`couple_profile`)
- `couple_id`(PK 겸 FK, couple 1:1), `couple_color`(HEX, 커플 공동 일정 표시색 — 양쪽 공유), `created_at`
- `CONNECTED` 전이 시 생성. (색상 모델 = 유저 개인색 + 공유 커플색, FR-7 참조.)
...
- **FR-1.6** Access Token TTL은 짧게(기준값 15분). **로그인·커플 연결(FR-2.3) 시 Access Token을 재발급**하여 `coupled` claim을 최신화한다. → 연결 직후 앱 잠금 해제를 최소 지연으로 반영.
...
- **FR-1.8** 내 정보 조회: 인증된 유저의 `public_id`, `nickname`, `email`, `couple_code`, `personal_color`, 현재 커플 상태(`NONE`/`PENDING`/`CONNECTED`)를 반환한다. **`couple_code`는 여기서 노출**하여 상대에게 공유한다(FR-2 연결의 전제). 내부 PK·`couple_id`는 응답에 포함하지 않는다(§1 식별자 정책).
- **FR-1.9** 커플 코드 재발급: 내 `couple_code`를 새 랜덤 코드로 교체한다(unique 충돌 시 재생성 재시도). 단, 진행 중인 커플 row(`PENDING`/`CONNECTED`, 만료 PENDING 제외)가 있으면 **409로 거절** — 코드 교체로 인한 연결 혼선 방지.
...
- **FR-2.5** 유저는 커플을 동시에 **하나만** 가진다(상태 무관 — `PENDING`/`CONNECTED` 통틀어, **단 만료 PENDING 제외**). 연결 시 **입력자(X)와 코드 주인(Y) 둘 다** 어떤 커플 row도 없어야 하며, 둘 중 한 명이라도 row가 있으면 연결 실패(이미 커플이면 애초에 튕긴다). → **서비스 단에서 보장**(만료 PENDING의 lazy 판정·H2 부분 인덱스 한계로 DB UNIQUE 강제가 까다로움).
...
- **FR-2.7** (PENDING 만료) `PENDING`은 생성(`created_at`) 후 **10분**이 지나면 만료된 것으로 본다. 만료된 PENDING은 (1) 커플 1개 제한(FR-2.5)·역방향 확인(FR-2.3) 판정에서 **없는 것으로 취급**하고(read 시점 lazy 판정), (2) row의 물리 정리는 **이번 스프린트 범위 외**(추후 정리 배치, TODO). → 잘못된 코드 입력·상대 무응답으로 인한 영구 잠금 방지. (lazy 판정 덕에 정리 배치가 없어도 새 연결은 가능하다.)

### FR-3 접근 게이트
- **FR-3.1** 일정 등록/조회 등 앱 기능 진입 전, JWT의 `coupled` claim으로 활성 커플 여부를 판정한다(보안 필터 단, DB 조회 없음).
- **FR-3.2** `coupled=false`(=`PENDING`만 있거나 커플 없음)이면 앱 기능 차단(403), 커플 연결만 허용.
- **FR-3.3** `coupled` claim은 토큰 발급 시점의 스냅샷이다. 상태 변화(연결)는 FR-1.6의 재발급 + TTL(≤15분)로 수렴한다.
...
### FR-6 헤어짐 / 삭제 — **이번 스프린트 범위 외 (제외)**
> 2인 전용 앱이라 헤어짐/커플 해제/데이터 삭제는 이번 스프린트에서 구현하지 않는다. "한번 연결되면 유지" 모델.
> 확장·공개 서비스로 가면 재검토 — README '안 하는 것' 참조. (FR 번호는 후속 문서 참조 안정성을 위해 비워 둔다.)
...
- **FR-7.3** `couple_profile`은 커플 `CONNECTED` 전이 시 생성.
